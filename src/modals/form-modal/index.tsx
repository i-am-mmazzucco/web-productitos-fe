import { useState } from "react";
import { FormProduct } from "../form-product";
import { FormPrice } from "../form-price";

interface FormModalProps {
  isOpen: boolean;
	setIsOpen: React.Dispatch<React.SetStateAction<boolean>>;
	setAddButton: React.Dispatch<React.SetStateAction<boolean>>;
	title: string;
	buttonName: string;
	type: 'price' | 'product';
}

export const FormModal: React.FC<FormModalProps> = ({ isOpen, setIsOpen, setAddButton, title, buttonName, type }) => {

	return (<>
	      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto overflow-x-hidden h-full w-full bg-gray-800 bg-opacity-50">
          <div className="relative w-full max-w-md p-4 max-h-full">
            <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
              <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
                  {title}
                </h3>
                <button 
                  type="button" 
                  onClick={() => {setIsOpen(false); setAddButton(false)}}
                  className="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                >
                  <svg className="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                    <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                  </svg>
                  <span className="sr-only">Cerrar formulario</span>
                </button>
              </div>

              {/* Modal Body Product */}
							{type ==='product' && (<FormProduct buttonName={buttonName} />)}

							{/* Modal Body Price */}
							{type ==='price' && (<FormPrice buttonName={buttonName} />)}
            </div>
          </div>
        </div>
      )}
	</>)
}